...
export const YarnDisplayCard = ({
  ...
  return <div className={`relative rounded-lg border transition-shadow shadow hover:shadow-md hover-scale cursor-pointer ring-0 border-gray-200 bg-white focus:outline-none ${className}`} tabIndex={0} {...props}>
      {/* Optional Edit/Delete buttons */}
      {(onEdit || onDelete) && <div className="absolute top-2 right-2 flex gap-2 z-20 pointer-events-auto">
          {onEdit && <Button size="icon" variant="ghost" onClick={e => {
        e.stopPropagation();
        onEdit(yarn);
      }} title="Edit yarn">
              <Edit className="w-4 h-4" />
            </Button>}
          {onDelete && <Button size="icon" variant="ghost" onClick={e => {
        e.stopPropagation();
        onDelete(yarn.id);
      }} title="Delete yarn">
      ...
      <div className="pt-6 pb-4 px-4">
        {yarn.image_url ? <div className="aspect-square rounded-lg overflow-hidden mb-2 bg-gray-100">
            <img src={yarn.image_url} alt={yarn.name} className="w-full h-full object-cover" />
          </div> : <div className="aspect-square rounded-lg bg-gray-100 flex items-center justify-center mb-2">
        ...
        <div className="flex flex-col gap-1 text-sm text-gray-600">
          ...
          {yarn.weight && <div className="flex flex-wrap items-center gap-2 w-full pt-1">
